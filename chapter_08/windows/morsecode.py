...
def is_validated_english_sentence(user_input):
    ...
    # """
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라
    # 자유로운 수정
    temp = list(user_input)
    forbid = list('''0123456789_@#$%^&*()-+=[]{}"';:\|`~''')
    for i in temp:
        if i in forbid:
            return False
    if get_cleaned_english_sentence(user_input) == '':
        return False
    return True
    # ==================================
...
def is_validated_morse_code(user_input):
    ...
    #       2) get_morse_code_dict 함수에 정의된 Morse Code 부호외 다른 코드가
    #       입력된 경우 ex)......
    # Examples:
    #     >>> import morsecode as mc
    ...
    morsecode = get_morse_code_dict()
    morsecode = list(morsecode.values())
    for i in user_input_list:
        if i in morsecode or i == '':
            pass
        else:
            return False
    return True
...
def get_cleaned_english_sentence(raw_english_sentence):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    temp = list(raw_english_sentence)
    remove_mark = list('.,?!')
...
def decoding_character(morse_character):
    # """
    # Input:
    #     - morse_character : 문자열값으로 get_morse_code_dict 함수로
    #       알파벳으로 치환이 가능한 값의 입력이 보장됨
    # Output:
...
def encoding_sentence(english_sentence):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    temp = get_cleaned_english_sentence(english_sentence).upper().split()
    encode_list = []
    # HI, FINE, THANK, YOU,
    for word in temp:
        encode_word = ''
        for alp in word:
            encode_word += encoding_character(alp)+' '
        encode_list.append(encode_word)
    result = ' '.join(encode_list).strip()
    return result
    # ==================================
